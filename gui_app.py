...
class RegresionLinealApp:
    def __init__(self, master):
        ...
        # Ocupa todo el ancho horizontal
        self.frame_contenedor_ruta.pack(fill=tk.X)
        ...
        self.boton_examinar = Button(
            self.frame_contenedor_ruta, text="Examinar", command=self.cargar_datos)
        self.boton_examinar.grid(row=0, column=1, padx=0, pady=5, sticky=tk.W)

        self.boton_cargar_modelo = Button(
            self.frame_contenedor_ruta, text="Cargar Modelo", command=self.cargar_modelo)
        self.boton_cargar_modelo.grid(
    ...
    def cargar_datos(self):
        ...
        file_path = filedialog.askopenfilename(initialdir="/", filetypes=[(
        ...
        if file_path:
            ...
            # Agrega esta línea para mostrar la tabla
    ...
    def mostrar_variables(self, variables):
        ...
        for radiobutton in self.radiobuttons_y:
            radiobutton.destroy()
        self.radiobuttons_x = self.crear_radiobuttons(
            self.frame_variables, variables, self.variable_x, row=2, column=1)
        self.radiobuttons_y = self.crear_radiobuttons(
            self.frame_variables, variables, self.variable_y, row=3, column=1)
    ...
    def crear_radiobuttons(self, frame, options, variable, row, column):
        ...
        for i, option in enumerate(options):
            radiobutton = tk.Radiobutton(
                frame, text=option, variable=variable, value=option)
            radiobutton.grid(row=row, column=column + i,
    ...
    def mostrar_tabla(self, df):
        ...
        self.tabla = ttk.Treeview(
            self.tabla_frame, show="headings", columns=clean_columns, height=6)
        ...
        self.scrollbar_y = Scrollbar(
            self.tabla_frame, orient=tk.VERTICAL, command=self.tabla.yview)
        ...
        self.tabla_frame.grid(row=0, column=0, columnspan=12,
                              padx=10, pady=5, sticky=tk.W)

        self.etiqueta_seleccionar = Label(
            self.frame_variables, text="Seleccionar variables:")
        self.etiqueta_seleccionar.grid(
            row=1, column=0, padx=10, pady=10, sticky=tk.W)

        self.etiqueta_variable_x = Label(
            self.frame_variables, text="Variable x:")
        self.etiqueta_variable_x.grid(
            row=2, column=0, padx=10, pady=5, sticky=tk.W)

        self.etiqueta_variable_y = Label(
            self.frame_variables, text="Variable y:")
        self.etiqueta_variable_y.grid(
            row=3, column=0, padx=10, pady=5, sticky=tk.W)

        self.boton_realizar_regresion = Button(
            self.frame_variables, text="Realizar regresión", command=self.realizar_regresion)
        self.boton_realizar_regresion.grid(
    ...
    def realizar_regresion(self):
        ...
        if self.variable_x.get() and self.variable_y.get():
            ...
            columnas_a_verificar = [
                self.variable_x.get(), self.variable_y.get()]
            ...
            ax.plot(X, y_pred, color='purple',
                    linewidth=2, label='Ajuste del modelo')
            ...
            # Utiliza la etiqueta de la variable X
            ax.set_xlabel(self.etiqueta_x)
            # Utiliza la etiqueta de la variable Y
            ...
            self.canvas = FigureCanvasTkAgg(
                self.figure, master=self.frame_variables)
            self.canvas.draw()
            self.canvas.get_tk_widget().grid(row=4, rowspan=20, column=2,
                                             columnspan=5, padx=10, pady=5, sticky=tk.W)

            # Agregar etiquetas de texto a la derecha del gráfico
            self.etiqueta_ecuacion = Label(
                self.frame_variables, text=f"La ecuación de la recta es: {self.ecuacion}")
            self.etiqueta_ecuacion.grid(
                row=4, column=7, columnspan=4, padx=10, pady=5, sticky=tk.W)
            self.etiqueta_datos = Label(
                self.frame_variables, text=f"El error cuadrático medio es: {self.mse:.4f} y la bondad de ajuste es: {self.r2:.4f}")
            self.etiqueta_datos.grid(
                row=5, column=7, columnspan=4, padx=10, pady=5, sticky=tk.W)

            self.etiqueta_descripcion = Label(
                self.frame_variables, text='Introduzca la descripción del modelo:')
            self.etiqueta_descripcion.grid(
                row=6, column=7, columnspan=2, padx=10, pady=5, sticky=tk.W)
            self.texto_descripcion = tk.StringVar()
            self.entrada_descripcion = tk.Entry(
                self.frame_variables, width=50, textvariable=self.texto_descripcion)
            self.entrada_descripcion.grid(
                row=6, column=9, columnspan=2, padx=10, pady=5, sticky=tk.W)

            # Nueva línea: Crear botón para guardar el modelo
            self.boton_guardar_modelo = Button(
                self.frame_variables, text="Guardar modelo", command=self.guardar_modelo)
            self.boton_guardar_modelo.grid(
            ...
            self.show_error(
                "Seleccione las variables x e y antes de hacer la regresión")
            raise ValueError(
                "Seleccione las variables x e y antes de hacer la regresión")
    ...
    def elementos_prediccion(self):
        ...
        self.frame_prediccion.grid(
            row=7, column=7, columnspan=4, padx=10, pady=5, sticky=tk.W)

        self.etiqueta_introducir_valor = Label(
            self.frame_prediccion, text=f"Introduzca un valor para {self.variable_x.get()}:")
        self.etiqueta_introducir_valor.grid(
            row=0, column=0, columnspan=2, padx=10, pady=5, sticky=tk.W)

        self.texto_valor_x = tk.StringVar()
        self.entrada_valor_x = tk.Entry(
            self.frame_prediccion, width=10, textvariable=self.texto_valor_x)
        self.entrada_valor_x.grid(
            row=0, column=2, padx=10, pady=5, sticky=tk.W)

        self.boton_realizar_prediccion = Button(
            self.frame_prediccion, text="Realizar predicción", command=self.realizar_prediccion)
        self.boton_realizar_prediccion.grid(
        ...
        if hasattr(self, 'info_modelo') and self.info_modelo is not None:
            try:
                ...
                self.etiqueta_nueva_ecuacion = Label(self.frame_prediccion, text=(
                    f"{self.info_modelo.variable_y} = {self.nueva_ecuacion}"))
                self.etiqueta_nueva_ecuacion.grid(
                    row=8, column=0, columnspan=4, padx=10, pady=5, sticky=tk.W)

            except ValueError as e:
                self.show_error(f"Error al realizar la predicción: {str(e)}")
            except Exception as e:
                self.show_error(
                    f"Error inesperado al realizar la predicción: {str(e)}")
    ...
    def guardar_modelo(self):
        ...
        try:
            ...
            file_path = filedialog.asksaveasfilename(defaultextension=".joblib", filetypes=[
    ...
    def cargar_modelo(self):
        ...
        if file_path:
            ...
            if not hasattr(self, 'etiqueta_nueva_ecuacion') or not self.etiqueta_nueva_ecuacion:
                self.etiqueta_nueva_ecuacion = Label(
                    self.frame_prediccion, text="")
                self.etiqueta_nueva_ecuacion.grid(
                    row=8, column=0, columnspan=4, padx=10, pady=5, sticky=tk.W)
            else:
                self.etiqueta_nueva_ecuacion.grid_forget()
    ...
    def mostrar_datos_modelo_cargado(self):
        ...
        if self.modelo_cargado is not None:
            ...
            label_coeficientes = tk.Label(
                self.frame_variables, text=f"Ecuación del modelo: {ecuacion_cargada}")
            label_coeficientes.grid(
                row=0, column=0, padx=10, pady=10, sticky=tk.W)
            label_intercepto = tk.Label(
                self.frame_variables, text=f"Error del modelo: {error_cargado}")
            label_intercepto.grid(
                row=1, column=0, padx=10, pady=10, sticky=tk.W)
            ...
            if descripcion_cargada is not None and descripcion_cargada != "":
                label_descripcion = tk.Label(
                    self.frame_variables, text=f"Descripción del modelo: {descripcion_cargada}")
